...
jQuery(function($) {
        ...
        top_menu_height = $('.dynamic-top-menu').height();
        // scroll spy to auto active the nav item
        $('body').scrollspy({ target: '#dynamic-nav-bar', offset: top_menu_height + 10 });
		$('.external-link').unbind('click');

        // scroll to top
        $('#btn-back-to-top').click(function(e){
            e.preventDefault();
            scrollTo('#dynamic-top');
        });

        // scroll to specific id when click on menu
        $('.dynamic-top-menu .navbar-nav a').click(function(e){
            e.preventDefault(); 
		...
		*/
        ...
        $('.dynamic-top-menu ').stickUp();

        // gallery category
        $('.dynamic-gallery-category a').click(function(e){
            e.preventDefault(); 
